    ...
    Session.findOne({ user: req.userData._id }).exec(function (err, record) {
      if (record) {
        ...
        if (record.usage.blocked && record.usage.nextAt > new Date()) {
          return cb({ error: 'user is blocked for session abuse and not ready for next usage', userMsg: 'user is blocked for session abuse', valid: false, record });
        }

        // session number exists
        const currentSession = record.getLogin(req.userData.urn);
        if (!currentSession) return cb({ error: 'login session number not found', userMsg: 'login session expired', valid: false, record });

        // session date is not expired
        if (isLoginExpired(currentSession)) return cb({ error: 'login session expired', userMsg: 'login session expired', valid: false, record });

        return cb({ error: null, valid: true, record });

      } else {
        return cb({ error: 'session record not found', userMsg: 'login session expired', valid: false, record });
      }
